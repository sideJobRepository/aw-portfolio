...
export default function PortfolioForm() {
  ...
  const [rooms, setRooms] = useState<
    Array<{
      ...
      desc: string;
      type: string;
      priceLow: { weekday: string; fri: string; sat: string }; // 비수기
      priceMid: { weekday: string; fri: string; sat: string }; // 준성수기
      priceHigh: { weekday: string; fri: string; sat: string }; // 성수기
    }>
  >([
    {
      id: "room-1",
      ...
      desc: "",
      type: "",
      priceLow: { weekday: "", fri: "", sat: "" },
      priceMid: { weekday: "", fri: "", sat: "" },
      priceHigh: { weekday: "", fri: "", sat: "" },
    },
  ]);
      ...
      if (question.isRequired) {
        ...
        if (question.questionType === "parlor") {
          ...
          const hasInvalidRoom = rooms.some((room) => {
            if (!room.name?.trim() || !room.desc?.trim() || !room.type?.trim())
              return true;
